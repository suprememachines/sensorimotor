...
def convert_direction(direction):
    return 1 if direction.lower() in ['counter', '1'] else 0


def set_direction(ser, board_id, direction):
    eat(ser)
    dint = convert_direction(direction)
    send_byte_sequence(ser, [80, board_id, dint]) # send set_id command 0x50
    return receive_byte_sequence(ser, [81, board_id]) # check for response 0x51
...
def main():
    ...
    parser.add_argument('-d', '--direction', default = 0, help = "(values 0/1, default: %(default)s)")
    ...
    with serial.Serial(args.port, baudrate, timeout=timeout_s) as ser:
        ...
        if args.board and check_board_id(args.board):
            ...
            if ping(ser, bid):
                ...
                if args.direction and check_board_id(args.board):
                    if ping(ser, bid):
                        print("Setting direction '{0}'".format(args.direction))
                        if set_direction(ser, bid, args.direction):
                            print("Succesfully set new direction.")
                    else:
                        print("Setting direction failed. Motor does not respond.")
